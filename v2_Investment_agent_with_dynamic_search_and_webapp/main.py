...
def main():
    ...
    # 1. Resolve ticker to full company name
    try:
        import yfinance as yf
        ticker_obj = yf.Ticker(company)
        full_name = ticker_obj.info.get('longName')
        if full_name:
            print(f"Resolved ticker {company} to full name: {full_name}")
            company_display_name = full_name
        else:
            company_display_name = company
    except Exception:
        company_display_name = company
        
    # JIT Ingestion: Automatically update ChromaDB with latest yfinance data
    try:
        from dynamic_ingest import ingest_single_ticker
        ingest_single_ticker(company, year)
    except Exception as e:
        print(f"Warning: Could not perform JIT ingestion for {company}: {e}")
    
    print(f"Starting research on: {company_display_name}" + (f" for year {year}" if year else ""))
    
    try:
        print("Retrieving financial context from ChromaDB...")
        # Query ChromaDB using the ticker symbol since dynamic_ingest uses the ticker
    ...
    # 3. Formulate the initial message embedding the retrieved context using the FULL company name
    initial_message = f"""Please draft an investment brief for {company_display_name} ({company}).
